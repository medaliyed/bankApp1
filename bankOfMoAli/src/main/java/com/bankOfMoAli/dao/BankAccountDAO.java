...
//    	this.entityManagersave(account);
...
//    }
    ...
    @Transactional(propagation = Propagation.REQUIRES_NEW, 
            rollbackFor = BankTransactionException.class)
	public void createAccount(Long id, String fullName, double balance) {
		System.out.println("created");
		BankAccount account = new BankAccount(id, fullName, balance);
		this.entityManager.createNativeQuery("INSERT INTO BANK_ACCOUNT (ID, FULL_NAME,BALANCE)" +
                "VALUES (?,?,?)")
                .setParameter(1, account.getId())
                .setParameter(2, account.getFullName())
                .setParameter(3, account.getBalance()).executeUpdate();
